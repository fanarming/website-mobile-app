module.exports = function(grunt) {
    ...
    var ASSETS_URL = 'http://assets.dwstatic.com/'+ pkg.name + '/'+ pkg.version + '/';
    ...
    grunt.initConfig({
        ...
        concat: {
            ...
            trans_module:{
                options: {
                    process: function(src, filepath) {
                        var regex = /(module\/[\w-]*.js)/g;
                        return src.replace(regex, ASSETS_URL+'js/'+'$1'+ '?t=' + new Date().getTime());
                    }
                },
                files: [{
                    expand: true,
    ...
    // 发布任务
    grunt.registerTask('release', ['sass', 'clean:build', 'copy:release', 'sprite', 'ejs_static:release','concat:trans_css','concat:trans_html','concat:trans_module','cssmin', 'uglify','clean:release','connect:site_dest']);
